// SALES TABLE 📊
// Final polished version: flush image + item, balanced spacing, tight padding
...
// Compact Slabfy logo for table display
...
export function SalesTable({ sales }: SalesTableProps) {
  ...
  const getVerificationIcon = (sale: any) => {
    const source = sale.source || 'ebay'
    const verified = sale.verified ?? true
    if (source === 'ebay' || verified) {
    ...
    return (
      <TooltipProvider delayDuration={0}>
        <Tooltip>
          ...
          <TooltipContent>
            <p className="text-xs">Cash payment - unverified transaction</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    )
  ...
  const calculatePrice = (sale: any) => {
    const salePrice = parseFloat(sale.sold_price?.value || sale.finalPrice || sale.price?.value || 0)
    const shippingCost = parseFloat(sale.shipping || 0)
    return { salePrice, shippingCost, total: salePrice + shippingCost }
  }
  ...
  return (
    <div className="rounded-md border">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className="w-[60px] px-2">Image</TableHead>
            <TableHead className="w-auto px-2">Item</TableHead>
            <TableHead className="w-[120px] px-2">Sold Price</TableHead>
            <TableHead className="w-[110px] px-2">Date</TableHead>
            <TableHead className="w-[90px] px-2">Verified</TableHead>
            <TableHead className="w-[120px] px-2">Marketplace</TableHead>
            <TableHead className="w-[60px] px-2 text-center">Link</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {sales.map((sale: any, index: number) => {
            const price = calculatePrice(sale)
            const imageUrl = sale.imageUrl || sale.image_url || sale.thumbnail
            const title = sale.title || 'No title'
            const soldDate = sale.sold_date?.date?.raw || sale.sold_date || sale.soldDate || ''
            ...
            return (
              <TableRow key={sale.id || index}>
                <TableCell className="py-2 px-2">
                  {imageUrl ? (
                    <img 
                      src={imageUrl} 
                      alt={title}
                      className="w-[50px] h-auto rounded-sm object-cover"
                      onError={(e) => {
                        const target = e.target as HTMLImageElement
                        target.outerHTML = '<span class="text-xs text-muted-foreground">No img</span>'
                      }}
                    />
                  ) : (
                  ...
                  <div className="space-y-0.5">
                    ...
                    {price.shippingCost > 0 && (
                      <div className="text-xs text-muted-foreground">
                        +${price.shippingCost.toFixed(2)} shipping
                      </div>
                    )}
                  </div>
                </TableCell>
                ...
                <TableCell className="py-2 px-2">
                  <div className="text-sm">{formatDate(soldDate)}</div>
                </TableCell>

                <TableCell className="py-2 px-2">
                  {getVerificationIcon(sale)}
                </TableCell>
